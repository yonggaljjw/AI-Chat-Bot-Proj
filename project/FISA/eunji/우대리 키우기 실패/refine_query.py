...
def refine_query(previous_text, query):
    # Combine the previous text and the query to provide context for refinement
    prompt = f"""
    사용자가 질문한 내용은 "{query}"이고, 이전 대화 내용은 {previous_text}이다.
    
    사용자가 질문한 내용을 바탕으로 궁극적인 의도를 파악해.
    만약 파악하지 못했을 때, 이 질문들을 종합적으로 분석하여 사용자의 궁극적인 의도를 파악하고, 하나의 최종 질문으로 만들어 주세요.

    Please follow these guidelines when answering:
    1. 사용자가 질문한 내용이 궁극적인 의도야.
    2. Questions about `오늘' are revised to `최근`.
    3. 최종 질문만 출력해.
    """

    # OpenAI API 호출 (모델은 GPT-4를 기본으로 설정)
    response = openai.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You are a helpful assistant skilled at consolidating and refining user questions."},
            {"role": "user", "content": prompt}
        ],
            max_tokens=150
    )
...
# 최종 응답
def answer_question_with_context(query, context=None, previous_messages=None):
    ...
    # 히스토리 가져오기
    previous_text = f"This is a conversation between a user and an AI assistant. The assistant remembers the recent conversation context.\n Recent conversation history:{previous_messages}" if context else ""
...
def chatbot_response(request):
    ...
    # POST 요청 처리 (기존 챗봇 대화)
        ...
        try:
            ...
            # 최신 3개의 대화 기록 가져오기
            previous_messages = "없음없음없음없음없음없음없음없음없음없음없음없음없음없음"
            previous_messages = get_latest_messages_by_session(session_id)
            previous_context = "\n".join([
                f"질문: {msg['message']}\n답변: {msg['response']}" for msg in reversed(previous_messages)
            ])
            print(previous_context)
            user_message_refine = refine_query(user_message, previous_context) # 사용자 질의 히스토리를 이용해 재정의
            print('사용자 질문 : '+ user_message)
            print('새로운 질문: '+user_message_refine)
            bot_response = f"{answer_question_with_context(user_message_refine, search_context, previous_context)}"
...
def get_latest_messages_by_session(session_id, limit=1):
    """
    주어진 세션 ID에 해당하는 최신 메시지(limit 개수) 반환 (message, response, timestamp 포함).
    """
    messages = ChatMessage.objects.filter(
        session_id=session_id
