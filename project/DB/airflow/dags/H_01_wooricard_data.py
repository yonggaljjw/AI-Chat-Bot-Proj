...
def upload_wooricard_data():
    file_path = '/opt/airflow/dags/data/edu_data_F_cleaned.csv'  # 실제 경로로 변경하세요.
...
with DAG(
    ...
    upload_wooricard_task = PythonOperator(
        task_id="upload_members_data_to_opensearch",
        python_callable=upload_wooricard_data,
    )
    ...
    # 작업 순서 설정
    upload_wooricard_data >> upload_age_payment_data >> upload_male_expense_data >> upload_female_expense_data >> upload_region_consumption_task >> upload_top_age_categories_data >> upload_top10_level_categories_data